...
export default class IdentityView {
  ...
  generateHeaderHTML(identity, firstName, lastName, preferredUsername) {
    ...
    return `
      <div class="lutece-identitypicker-container-header">
        <div>
          <h3 class="lutece-identitypicker-truncate">${firstName} ${lastName}${preferredUsername ? ` (${preferredUsername})` : ''}</h3>
          <p class="lutece-identitypicker-truncate lutece-identitypicker-description">${identity.customer_id}</p>
            ${showValidateEmailButton ? `<button class="lutece-identitypicker-validate-email-btn lutece-identitypicker-button-mini lutece-identitypicker-button-yellow">⚠️ ${this.identityPicker.rules.language.validateEmailButton}</button>` : ''}
        </div>
      <div class="lutece-identitypicker-container-header-btns">
        <button class="lutece-identitypicker-history-btn">${this.identityPicker.rules.language.historyButton}</button>
        ${this.identityPicker.permissions.update ? `<button class="lutece-identitypicker-modify-btn">${this.identityPicker.rules.language.modifyButton}</button>` : ''}
        ${Object.keys(this.identityPicker.config.fieldMapping || {}).length > 0 && this.identityPicker.config.selection ? `<button class="lutece-identitypicker-select-btn" ${!isSelected ? 'disabled' : ''}>${!isSelected ? `${this.identityPicker.rules.language.selectedButton}` : `${this.identityPicker.rules.language.selectButton}`}</button>` : ''}
      </div>
      </div>
      <div class="lutece-identitypicker-info-tags">
        <span class="lutece-identitypicker-info-tag lutece-identitypicker-tag-default">
        ...
        <span class="lutece-identitypicker-info-tag ${identity.mon_paris_active ? 'lutece-identitypicker-tag-success' : 'lutece-identitypicker-tag-error'}">
          <strong>${this.identityPicker.rules.language.monParisAccount}</strong> ${identity.mon_paris_active ? this.identityPicker.rules.language.active : ` ${showCreateAccountButton ? `<button class="lutece-identitypicker-create-account-btn lutece-identitypicker-button-mini lutece-identitypicker-button-red">${this.identityPicker.rules.language.createAccountButton}</button>` : this.identityPicker.rules.language.inactive}`}
        </span>
        <span class="lutece-identitypicker-info-tag ${this.getCoverageClass(identity.quality.coverage)}">
  ...
  generateAttributesTable(identity) {
    ...
    for (const [groupKey, attrs] of Object.entries(groupedAttributes)) {
      if (attrs.length > 0) {
        ...
        tableHTML += `
          <tr class="lutece-identitypicker-group-separator">
            <td class="lutece-identitypicker-table-separator" colspan="3">${groupLabel}</td>
          </tr>`;
  ...
  getCoverageClass(coverage) {
    if (coverage < 0.5) return 'lutece-identitypicker-tag-error';
    if (coverage < 0.8) return 'lutece-identitypicker-tag-warning';
    return 'lutece-identitypicker-tag-success';
  }
  ...
  setupButtons(identity) {
    const detailsContainer = this.identityPicker.detailsContainer;
    const modifyButton = detailsContainer.querySelector('.lutece-identitypicker-modify-btn');
    const selectButton = detailsContainer.querySelector('.lutece-identitypicker-select-btn');
    const historyButton = detailsContainer.querySelector('.lutece-identitypicker-history-btn');
    const createAccountButton = detailsContainer.querySelector('.lutece-identitypicker-create-account-btn');
    const validateEmailButton = detailsContainer.querySelector('.lutece-identitypicker-validate-email-btn');
