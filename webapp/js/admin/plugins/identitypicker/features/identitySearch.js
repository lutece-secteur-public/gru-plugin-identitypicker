...
export default class IdentitySearch {
  ...
  async initSearchView() {
    ...
    searchContainer.innerHTML = `
      <div class="lutece-identitypicker-search-option">
        <label>
      ...
      <form id="lutece-identitypicker-email-form-${this.uniqueId}" style="display: none;">
        <input type="email" placeholder="${this.identityPicker.rules.language.emailPlaceholder}" name="email" required>
        <div class="lutece-identitypicker-container-buttons">
          <button class="lutece-identitypicker-button-light" type="button">${this.identityPicker.rules.language.closeButton}</button>
          <button type="submit">${searchIcon} ${this.identityPicker.rules.language.searchButton}</button>
        </div>
      </form>
      <form id="lutece-identitypicker-name-form-${this.uniqueId}">
    ...
    this.emailForm = document.getElementById(`lutece-identitypicker-email-form-${this.uniqueId}`);
    this.nameForm = document.getElementById(`lutece-identitypicker-name-form-${this.uniqueId}`);
  ...
  displayResults(results) {
    ...
    resultsContainer.innerHTML = `<ul class="lutece-identitypicker-results-list">${resultsHtml}</ul>`;
    this.addCreateIdentityButton();
  ...
  createResultItem(result) {
    ...
    const qualityStats = `
      <span class="lutece-identitypicker-info-tag ${this.getBadgeColorClass(parseFloat(scoringPercentage))}">
        <strong>${this.identityPicker.rules.language.qualityScore}</strong> ${scoringPercentage}%
      </span>
    `;

    return `
      <li class="lutece-identitypicker-result-item" data-customer-id="${result.customer_id}">
        <div class="lutece-identitypicker-result-left">
          <div class="lutece-identitypicker-result-header">
            <h3>${nameDisplay}</h3>
          </div>
          <div class="lutece-identitypicker-result-details">
            <p>${detailsDisplay}</p>
            <p>
        ...
        <div class="lutece-identitypicker-result-right">
          ${qualityStats}
        </div>
      </li>
    `;
  ...
  addCreateIdentityButton() {
    ...
    buttonContainer.classList.add('lutece-identitypicker-container-buttons');
    ...
    newSearchButton.classList.add('lutece-identitypicker-button-light');
    ...
    createButton.classList.add('lutece-identitypicker-create-button');
  ...
  async setupResultsClickHandler() {
    this.identityPicker.resultsContainer.addEventListener('click', (event) => {
      const resultItem = event.target.closest('.lutece-identitypicker-result-item');
      if (resultItem) {
  ...
  getBadgeColorClass(percentage) {
    if (percentage >= 80) return 'lutece-identitypicker-tag-success';
    if (percentage >= 50) return 'lutece-identitypicker-tag-warning';
    return 'lutece-identitypicker-tag-error';
  }

  getColorClass(percentage) {
    if (percentage >= 80) return 'lutece-identitypicker-high';
    if (percentage >= 50) return 'lutece-identitypicker-medium';
    return 'lutece-identitypicker-low';
  }

  getSearchData() {
    const nameForm = document.getElementById(`lutece-identitypicker-name-form-${this.uniqueId}`);
    return {
  ...
  highlightDifferences(search, result) {
    ...
    for (let i = 0; i < result.length; i++) {
      if (searchIndex < searchNormalized.length && 
        ...
        if (!inHighlight) {
          highlightedResult += '<span class="lutece-identitypicker-highlight">';
          inHighlight = true;
        }
        highlightedResult += result[i];
      }
  ...
  highlightLastNameDifferences(search, lastName, preferredUsername) {
    ...
    const lastNameDiffCount = (lastNameMatch.match(/<span class="lutece-identitypicker-highlight">/g) || []).length;
    const preferredUsernameDiffCount = (preferredUsernameMatch.match(/<span class="lutece-identitypicker-highlight">/g) || []).length;

    if (lastNameDiffCount <= preferredUsernameDiffCount) {
      return `${lastNameMatch} <span class="lutece-identitypicker-preferred-name">(${preferredUsername})</span>`;
    } else {
      return `${lastName} <span class="lutece-identitypicker-preferred-name">(${preferredUsernameMatch})</span>`;
    }
  }
