...
export default class IdentityForm {
    ...
    async displayForm(mode) {
        const formId = `lutece-identitypicker-${mode}-form-${this.uniqueId}`;
        let formHtml = `<form id="${formId}">`;

        for (const [groupKey, group] of Object.entries(ATTRIBUTE_GROUPS)) {
            formHtml += `<fieldset class="lutece-identitypicker-fieldset">
        ...
        formHtml += `<div class="lutece-identitypicker-container-buttons">
        <button class="lutece-identitypicker-button-back lutece-identitypicker-button-light" type="button">${this.identityPicker.rules.language.backButton}</button>
        ...
        this.identityPicker.createIdentityContainer.querySelector('.lutece-identitypicker-button-back').addEventListener('click', () => {
            if (mode === 'create') {
    ...
    generateAttributeField(attrKey) {
        ...
        return `
      <div class="lutece-identitypicker-form-row">
        <div class="lutece-identitypicker-form-input">
          <label for="${attr.keyName}">${attr.name} ${attr.attributeRight.mandatory ? `<span class="lutece-identitypicker-required">${this.identityPicker.rules.language.mandatory}</span>` : ''}</label>
          ${this.getInputFieldHtml(attr, value)}
        </div>
        <div class="lutece-identitypicker-form-select">
          <label for="${attr.keyName}-certification">${this.identityPicker.rules.language.selectCertification} ${attr.attributeRight.mandatory ? `<span class="lutece-identitypicker-required">${this.identityPicker.rules.language.mandatory}</span>` : ''}</label>
          <select id="${attr.keyName}-certification" name="${attr.keyName}-certification" class="lutece-identitypicker-select" ${attr.attributeRight.mandatory ? 'required' : ''}>
    ...
    getInputFieldHtml(attr, value) {
        if (attr.keyName === 'birthcountry') {
            return `
        <select id="birthcountry" name="birthcountry" class="lutece-identitypicker-birthcountry-select" ${attr.attributeRight.mandatory ? 'required' : ''}>
          ${value ? `<option value="${value}" selected>${value}</option>` : ''}
            ...
            if (attributeKey && attributeKey.values && attributeKey.values.length > 0) {
                return `
          <select id="${attr.keyName}" class="lutece-identitypicker-select" name="${attr.keyName}" ${attr.attributeRight.mandatory ? 'required' : ''}>
    ...
    initializeChoices() {
        document.querySelectorAll('.lutece-identitypicker-select').forEach(select => {
            new Choices(select, {
                searchEnabled: false,
                itemSelectText: '',
                shouldSort: false,
    ...
    initBirthcountrySelect() {
        const birthcountrySelect = document.querySelector('.lutece-identitypicker-birthcountry-select');
        if (birthcountrySelect) {
    ...
    displayFieldErrors(attributesStatus) {
        ...
        attributesStatus.forEach(attribute => {
            ...
            if (inputField) {
                ...
                errorMessage.className = 'lutece-identitypicker-field-error';
                errorMessage.textContent = attribute.message;
                inputField.parentNode.insertBefore(errorMessage, inputField.nextSibling);
                inputField.classList.add('lutece-identitypicker-error-input');
            }
        });
    }

    clearFieldErrors() {
        const errorMessages = this.identityPicker.createIdentityContainer.querySelectorAll('.lutece-identitypicker-field-error');
        errorMessages.forEach(msg => msg.remove());

        const errorInputs = this.identityPicker.createIdentityContainer.querySelectorAll('.lutece-identitypicker-error-input');
        errorInputs.forEach(input => input.classList.remove('lutece-identitypicker-error-input'));
    }
